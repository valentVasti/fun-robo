...
document.addEventListener("DOMContentLoaded", function () {
  ...
  switch (table.dataset.page) {
    ...
    case "testimoni":
      config = {
        data_key: ["nama_testimoni", "keterangan_testimoni", "umur_testimoni", "isi_testimoni"],
        page: "testimoni",
    ...
    case "article":
      config = {
        data_key: ["judul", "penulis", "thumbnail", "tag", "highlighted"],
        page: "article",
        url: "article/search/",
    ...
    case "branch":
      config = {
        data_key: ["nama_branch", "phone_num", "email", "informasi_detail"],
        page: "branch",
        url: "branch/search/",
    ...
    case "curriculum":
      config = {
        data_key: ["curriculum_name", "price", "duration", "age_min", "age_max", "description"],
        page: "curriculum",
  ...
  search.addEventListener("input", function () {
    ...
    fetch(url + value).then(function (response) {
      ...
      var notFound = document.getElementById("not-found-text");
      ...
      if (table) {
        table.style.display = "";
        var rows = table.querySelectorAll("tr[id]");
        var resultRow = table.getElementsByClassName("search-result");
        var rowCount = rows.length;
        for (var i = resultRow.length - 1; i >= 0; i--) {
          var result = resultRow[i];
          result.parentNode.removeChild(result);
        }
        for (var _i = rowCount - 1; _i >= 0; _i--) {
          console.log(_i);
          var row = rows[_i];
          console.log(row);
          row.style.display = "";
  ...
  function setSearchTable(data, value) {
    ...
    if (dataStatus == "Found") {
      ...
      var total = element.children[3];
      total.style.display = "";
      if (searchTotal == null) {
        var _searchTotal = document.createElement("span");
        _searchTotal.id = "search-total";
        _searchTotal.innerHTML = "Found and showing " + resultArr.length + " results";
        total.children[0].style.display = "none";
        total.append(_searchTotal);
      } else {
        searchTotal.innerHTML = "Found and showing " + resultArr.length + " results";
        total.children[0].style.display = "none";
      }
      resultArr.forEach(function (element) {
        ...
        config.data_key.forEach(function (key) {
          ...
          if (key == "created_at" || key == "updated_at") {
            ...
            deleteBtn.setAttribute("onclick", config.deleteFunction + "('" + element["id"] + "')");
            ...
            tagIcon.classList.add("fa-solid", "fa-tag", "tag-detail", "pointer");
            ...
            tagArr.forEach(function (tag_data) {
              var spanTag = document.createElement("span");
              spanTag.classList.add("attribute-modal");
              spanTag.innerText = tag_data["detail"]["tag_name"];
              swalHtml.appendChild(spanTag);
            });
            var swalFunctionParam = document.createElement("swal-function-param");
            swalFunctionParam.setAttribute("name", "showConfirmButton");
            ...
            locationIcon.classList.add("fa-solid", "fa-house", "pointer");
            ...
            facebookIcon.classList.add("fa-brands", "fa-facebook", "fa-xl");
            ...
            var swalHtml = document.createElement("swal-html");
            var judul = document.createElement("h3");
            judul.innerHTML = "Detail Lokasi " + element["nama_branch"];
            var br = document.createElement("br");
            ...
            swalHtml.append(judul, br.cloneNode(), alamatLabel, alamatData, br.cloneNode(), kotaLabel, kotaData, br.cloneNode(), provinsiLabel, provinsiData, br.cloneNode());
            var swalFunctionParam = document.createElement("swal-function-param");
            swalFunctionParam.setAttribute("name", "showConfirmButton");
            swalFunctionParam.setAttribute("value", "false");
            ...
            tdData.append(locationSpan, " ", instagramSpan, " ", facebookSpan);
          } else if (key == "highlighted") {
            tdData.classList.add("text-center", "pointer");
            var highlightedValue = element[key];
            var iconElement = document.createElement("i");
            iconElement.classList.add("fa-solid", "fa-star", "highlighted");
            iconElement.setAttribute("onclick", "updateHighlight(this, " + element["id"] + ")");
            iconElement.setAttribute("data-highlight", highlightedValue);
      ...
      if (notFound == null) {
        var _notFound = document.createElement("p");
        _notFound.classList.add("text-center");
        _notFound.id = "not-found-text";
        _notFound.innerHTML = '0 result for "' + value + '"';
        _notFound.style.fontSize = "x-large";
        element.append(_notFound);
      } else {
  ...
  function formatDateTime(datetime) {
    // PHP datetime string
    var phpDateTime = datetime;

    // Parse the PHP datetime string into a JavaScript Date object
    var dateObj = new Date(phpDateTime);
